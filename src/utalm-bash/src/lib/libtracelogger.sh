...
#$Header$
#
if [ -z "$__TRACELOGGER__" ];then #*** prevent multiple inclusion
__TRACELOGGER__=1 #*** prevent multiple inclusion

if [ -e ${BASH_SOURCE%/*}/bootstrap/bootstrap-01.01.009.sh ];then
	. ${BASH_SOURCE%/*}/bootstrap/bootstrap-01.01.009.sh
else
	. ${BASH_SOURCE%/*}/../bin/bootstrap/bootstrap-01.01.009.sh
fi
. ${BASH_SOURCE%/*}/core/libcore-01.01.009.sh

_myLIBNAME_tracelogger="${BASH_SOURCE}"
_myLIBVERS_tracelogger="02.01.001"
...
if [ ! -d "${MYLIBPATH}" -o ! -e "${MYLIBPATH}/libtracelogger.sh" ];then
...
function fetchDBGArgs () {
	...
	for i in ${C_DARGS//[,()]/ };do
		...
		H|HELP)
		    cat <<EOF
UnifiedTraceAndLogManager-bash: libtracelogger.sh
