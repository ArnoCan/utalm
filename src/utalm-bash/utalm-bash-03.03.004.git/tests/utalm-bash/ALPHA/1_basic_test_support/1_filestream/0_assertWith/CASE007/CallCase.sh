...
export dataX
{
	( TESTMODE=$T_CREATEREF assertRefDataWithExit $LINENO $BASH_SOURCE ID000 "$dataX" )
	TESTMODE=$T_COMPAREREF  assertRefDataWithExit $LINENO $BASH_SOURCE ID000 "$dataX"
} 2>&1 |countErrors flat=1 filter=1 sums=1 intermediate=1
