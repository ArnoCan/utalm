...
##
## Loads the Nodeaction "src", refer to "Makefile-nodewalk.mk".
## Implements the action src preparation for performing.
##
## \cond
##
#
...
#
#
ifndef _MAKE_NODEWALK_NODEACTION_SRC_INCLUDED_
_MAKE_NODEWALK_NODEACTION_SRC_INCLUDED_:=1

#NODEACTIONONLY=1
POSTFIXACTION=stripped
...
stripped: $(SRC_FILES) $(SRC_DIRS)
	strip-bash.sh DROPLINES -o $(RTBASE)bin $(RTBASE)bin/strip-bash.sh
	strip-bash.sh DROPLINES -o $(RTBASE)bin $(RTBASE)bin/setversion.sh
	strip-bash.sh DROPLINES -o $(RTBASE)lib $(RTBASE)lib/libutalm.sh
	strip-bash.sh DROPLINES -o $(RTBASE)lib $(RTBASE)lib/libutalm.awk
	strip-bash.sh DROPLINES -o $(RTBASE)lib $(RTBASE)lib/libutalmfileobjects.sh
	strip-bash.sh DROPLINES -o $(RTBASE)lib $(RTBASE)lib/libutalmrefpersistency.sh
