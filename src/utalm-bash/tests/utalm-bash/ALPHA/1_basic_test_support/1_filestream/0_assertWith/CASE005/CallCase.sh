...
refdata=$(printRefData $S_ALL $D_ON $LINENO $BASH_SOURCE $id 2>&1|sed -n '/^BLOB_START/,/^BLOB_END/!d;/^BLOB_/d;p')
assert $LINENO $BASH_SOURCE "[[ '$data' == '$refdata' ]]"
#
assertRefDataWithExit $LINENO $BASH_SOURCE $id "$data"
