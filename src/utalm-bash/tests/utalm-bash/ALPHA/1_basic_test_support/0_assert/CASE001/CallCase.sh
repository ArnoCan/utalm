...
tstExec=$(getPathToBin.sh bin/exec4ReturnError.sh)
r=$(utalm-bash-cli.sh -d f:callname%filename%subsysstr%nocolor callErrOutWrapper $LINENO $BASH_SOURCE "$tstExec -x -z 2" 2>&1)
x=$?
assert $LINENO $BASH_SOURCE "[[ $x == 222 ]]"
assert $LINENO $BASH_SOURCE "[[ '$r' == '-x -z 2' ]]"
