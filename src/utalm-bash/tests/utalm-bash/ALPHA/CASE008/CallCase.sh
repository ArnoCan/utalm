...
((DBG>0))&&echo "TEST008"
((DBG>0))&&echo
((DBG>0))&&echo 'printFINALCALL $D_FLOW $LINENO $BASH_SOURCE TEST004 "$MYCALLPATH/exec4ReturnError.sh -x -z 2"'
a=b
printFINALCALL $D_FLOW $LINENO $BASH_SOURCE TEST004 "$MYCALLPATH/exec4ReturnError.sh -x -z 2"
ret=$?
if [ $? -ne 1 ];then
